...
(defn render-metadatas []
  [:div.book-metadatas
   [:div.d-flex.flex-row.w-100.justify-content-between
    [:h1 {:style {:font-weight 600 :color "#303030"}} (.-title @(rf/subscribe [:selected-book]))]
...
;     [ant/button {:class-name "mt-3" :type "secondary" :on-click #(rf/dispatch [:toggle-recommendation-modal true])} "View Similar Books"]]
   ...
   [:div.d-flex.align-items-center.mb-3
    [ant/rate {:disabled true :allow-half true :value (.-average @(rf/subscribe [:selected-book-reviews]))}]
    [:span {:class-name "ant-rate-text pt-1" :style {:color "gray"}} (str (.-average @(rf/subscribe [:selected-book-reviews])) " out of 5 from " (.-total @(rf/subscribe [:selected-book-reviews])) " review(s)")]
    ]
   [:div.d-flex.align-items-center.mb-2
    [:span {:style {:color "#303030" :font-weight 500 :margin-right "1%"}} "Author:"]
    [:span {:style {:color "gray"}} (.-author @(rf/subscribe [:selected-book]))]
    ]
   [:div.d-flex.align-items-center.mb-2
    [:span {:style {:color "#303030" :font-weight 500 :margin-right "1%"}} "Genre:"]
    (doall (for [i (range (.-length (.-categories @(rf/subscribe [:selected-book]))))] (reagent/as-element [ant/tag {:key (aget (.-categories @(rf/subscribe [:selected-book])) i)} (aget (.-categories @(rf/subscribe [:selected-book])) i)])))
    ]
...
(defn render-reviews []
  (if @(rf/subscribe [:selected-book-reviews])
    [ant/list {:itemLayout "vertical" :pagination {:page-size 3} :dataSource (.-reviewsList @(rf/subscribe [:selected-book-reviews])) :render-item (reagent/as-element render-review-item)}]
    [ant/list {:itemLayout "vertical" :pagination {:page-size 3} :loading true :render-item (reagent/as-element render-review-item)}]
    )
  )
    ...
    (go (let [response (<! (http/post (str tableutils/proxy_url (macro/review_url) "/api/create_review")
                                      {:with-credentials? false
                                       :json-params {:asin   (.-asin @(rf/subscribe [:selected-book]))
                                                     ...
                                                     :reviewText (.-description values)
                                                     :summary (.-summary values)
                                                     :unixReviewTime (.floor js/Math (/(.getTime (js/Date.)) 1000))
                                                     :reviewTime (str (+ 1 (.getMonth (js/Date.))) " " (.getDate (js/Date.)) ", " (.getFullYear (js/Date.)))
                                                     :reviewerID "ZZ9794643"
                                                     :reviewerName "Daryll Wong"
                                                     :helpful "[2,9]"
                                                     }
                                       }
                                      ))]
          (def create-review-response (.-body (.parse js/JSON (:body response))))
      ...
      [ant/form {:on-submit #(do (.preventDefault %) (submit-handler)) :class-name "add-review-form"}
       [:div.d-flex.flex-row
        [:div.d-flex.flex-column.mr-3
         [:img {:src (.-imUrl @(rf/subscribe [:selected-book])) :style {:object-fit "cover" :width "383px"}}] ;;242px
         ]
        [:div.d-flex.flex-column.w-100
         [:h3 {:style {:font-weight 600 :color "#303030" :margin-bottom 0}} (.-title @(rf/subscribe [:selected-book]))]
         [ant/form-item
...
(defn render-recommendations []
  [:div.d-flex.scrolling {:style {:overflow "auto"}}
   (if (not @(rf/subscribe [:related-books]))
     [:div.d-flex.align-items-center.justify-content-center.w-100.h-100
      [ant/spin]
      ]
     (if (empty? (js->clj (.-asinList @(rf/subscribe [:related-books]))))
       [:div.d-flex.align-items-center.justify-content-center.w-100.h-100
        [:h6 {:style {:color "gray" :font-weight 400} :class-name "mt-3"} "No Recommended Books Available"]
        ]
       (doall
        (for [i (range (.-length (.-asinList @(rf/subscribe [:related-books]))))]
          [:a {:href (str "/#/view-book/" (aget (.-asinList @(rf/subscribe [:related-books])) i)) :key (aget (.-asinList @(rf/subscribe [:related-books])) i)}
